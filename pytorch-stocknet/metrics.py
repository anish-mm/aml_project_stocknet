...
def get_true_yT(y_true, n_days):
    sample_index = torch.arange(y_true.shape[0]).unsqueeze(-1)
    day_index = (n_days - 1).unsqueeze(-1)
    y_T_ = y_true[sample_index, day_index].squeeze(1)
    return y_T_
...
def n_accurate(y, y_):
    ...
    """
    correct_y_batch = torch.eq(torch.argmax(y, 1), torch.argmax(y_, 1))
    n_accurate = correct_y_batch.float().sum()  # similar to numpy.count_nonzero()
    return n_accurate
...
def eval_res(gen_n_acc, gen_size, y_list, y_list_, use_mcc=None):
