...
parser.add_argument("-td", "--tensorboard-dir", type=str, default="../runs", help="path to save tensorboard summaries at",
                    dest="tb_path")
parser.add_argument("-nr", "--no-resume", dest="resume", action="store_false",
                        help="use if model not to be loaded from checkpoint for training")
...
class Executor:
    ...
    def train_and_dev(self):
        with tf.Session(config=self.tf_config) as sess:
            ...
            tensorboard_writer = tf.summary.FileWriter(args.tb_path)
            acc_tensor = tf.Variable(0, dtype=tf.float32)
            loss_tensor = tf.Variable(0, dtype=tf.float32)
            epoch_acc_tensor = tf.Variable(0, dtype=tf.float32)
            epoch_loss_tensor = tf.Variable(0, dtype=tf.float32)
            dev_acc_tensor = tf.Variable(0, dtype=tf.float32)
            dev_loss_tensor = tf.Variable(0, dtype=tf.float32)

            acc_summ = tf.summary.scalar('Batch Accuracy', acc_tensor)
            loss_summ = tf.summary.scalar('Batch Loss', loss_tensor)
            epoch_acc_summ = tf.summary.scalar('Epoch Accuracy', epoch_acc_tensor)
            epoch_loss_summ = tf.summary.scalar('Epoch Loss', epoch_loss_tensor)
            dev_acc_summ = tf.summary.scalar('Dev Accuracy', dev_acc_tensor)
            dev_loss_summ = tf.summary.scalar('Dev Loss', dev_loss_tensor)
            ...
            if checkpoint and checkpoint.model_checkpoint_path and args.resume:
            ...
            for epoch in range(self.model.n_epochs):
                ...
                for train_batch_dict in train_batch_gen:
                    ...
                    # calls `model.optimize` on with `feed_dict` as input. returns values of tensors in ops.
                    ...
                    sess.run(acc_tensor.assign(train_batch_n_acc / train_batch_dict["batch_size"])) # update accuracy variable
                    sess.run(loss_tensor.assign(train_batch_loss))
                    tensorboard_writer.add_summary(sess.run(acc_summ), n_iter) #
                    tensorboard_writer.add_summary(sess.run(loss_summ), n_iter)
                    ...
                    if n_iter >= self.silence_step and n_iter % self.skip_step == 0:
                        ...
                        sess.run(dev_acc_tensor.assign(res["acc"]))
                        sess.run(dev_loss_tensor.assign(res["loss"]))
                        tensorboard_writer.add_summary(sess.run(dev_acc_summ), n_iter) #
                        tensorboard_writer.add_summary(sess.run(dev_loss_summ), n_iter)
                ...
                # save to tensorboard
                sess.run(epoch_loss_tensor.assign(epoch_loss))
                sess.run(epoch_acc_tensor.assign(epoch_acc))
                tensorboard_writer.add_summary(sess.run(epoch_acc_summ), epoch) #
                tensorboard_writer.add_summary(sess.run(epoch_loss_summ), epoch)
    ...
    def restore_and_test(self):
        with tf.Session(config=self.tf_config) as sess:
            ...
            if checkpoint and checkpoint.all_model_checkpoint_paths:
                d = dict()
                for pp in checkpoint.all_model_checkpoint_paths:
                    logger.info('Model: {0}, session restored!'.format(self.model.model_name))
                    self.saver.restore(sess, checkpoint.model_checkpoint_path)
                    res = self.generation(sess, phase='test')
                    d[pp] = res
