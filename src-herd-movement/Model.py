...
class Model:
    ...
    def __init__(self):
        ...
        self.max_n_days = config_model['max_n_days'] # max number of days per sample
        self.max_n_msgs = config_model['max_n_msgs'] # max number of msgs per day
        self.max_n_words = config_model['max_n_words'] # max number of words per message
        ...
        self.price_embed_size = config_model['word_embed_size'] # seems unused

        self.mel_cell_type = config_model['mel_cell_type'] # basic, gru, ln-lstm
        self.variant_type = config_model['variant_type'] # hedge? discriminative? fundamental? ...
        self.vmd_cell_type = config_model['vmd_cell_type'] # basic, gru, ln-lstm

        self.vmd_rec = config_model['vmd_rec'] # not sure what this is.
    ...
    def _build_placeholders(self):
        with tf.name_scope('placeholder'):
            ...
            """
            Edited since we need 6 price variables.
            """
            # self.price_ph = tf.placeholder(dtype=tf.float32, shape=[None, self.max_n_days, 3])  # high, low, close
            self.price_ph = tf.placeholder(dtype=tf.float32, shape=[None, self.max_n_days, 6])  # high, low, close
    ...
    def _build_embeds(self):
        r"""
        Anish : This is building the embeddings array for the current set of words whose ids are in self.word_ph
        """
        with tf.name_scope('embeds'):
            with tf.variable_scope('embeds'):
                """
                Anish : word table will be a lookup table for words and their embeddings. Not trainable while training
                the prediction model. Probably set in some pre-training step. Has shape (vocab size, embedding size)
                """
                word_table = tf.get_variable('word_table', initializer=self.word_table_init, trainable=False)
                """
                Anish : word_table will be a lookup table mapping words to corresponding embeddings.
                here, we are building word_embed by looking up the embeddings for ids in word_ph. 
                """
                self.word_embed = tf.nn.embedding_lookup(word_table, self.word_ph, name='word_embed')

    def _create_msg_embed_layer_in(self):
        """
            Add Batch normalization (if needed), and dropout to the input to the message embedding layer.
        ...
        """
        """
        Anish : This is the input to the Message Embedding Layer(MEL). Takes the word_embed, applies batch_norm if needed,
        add dropout.
        """
    ...
    def _create_msg_embed_layer(self):
        """
            ...
            Input:
                mel_in: same as word_embed (batch_size * max_n_days * max_n_msgs * max_n_words * word_embed_size)

            Output:
                msg_embed: batch_size * max_n_days * max_n_msgs * msg_embed_size
        """
        """
        Anish : Combine the word embeddings in the message to get one embedding for the whole message.
        """
        ...
        def _for_one_trading_day(daily_in, daily_ss_index_vec, daily_mask):
            """
                Input:
                    ...
                    daily_ss_index_vec: max_n_msgs
                    daily_mask: max_n_msgs
                        actual length for the messages. eg; i^th value holds the actual length of i^th message.
                    daily_ss_index_vec:  what is this ? not exactly sure. it is used to index into the message embeddings. why?
                Output:
                    msg_embed: max_n_msgs * msg_embed_size
            """
            # here out will be a tuple of forward, and backward rnn output tensors.
            # create bidirectional model with the cells created.
            ...
            out_id = tf.concat([msg_ids, ss_indices], axis=1) # [[0, id1], [1, id2], [2, id3], ...]
            # some indexing is happening
            mel_h_f, mel_h_b = tf.gather_nd(out_f, out_id), tf.gather_nd(out_b, out_id)
            # average forward, and backward embeddings.
            msg_embed = (mel_h_f + mel_h_b) / 2

            return msg_embed

        def _for_one_sample(sample, sample_ss_index, sample_mask):
            # for each day in the sample, apply '_for_one_trading_day'
            # sample_ss_index: max_n_days * max_n_msgs
            # sample_mask: max_n_days * max_n_msgs
        ...
        def _for_one_batch():
            # apply `for_one_sampe` to each sample in the batch iteratively.
            # `mel_in` is the word embeddings tensor for the batch of messages. (Check function comment above for shape.)
            # n_words_ph : batch_size * max_n_days * max_n_msgs
            # ss_index_ph : batch_size * max_n_days * max_n_msgs
        ...
        with tf.name_scope('mel'):
            with tf.variable_scope('mel_iter', reuse=tf.AUTO_REUSE):
                # set up the rnn/lstm/gru cells for forward and backward.
                ...
                # shape of message embeddings after combining word embeddings to form message embeddings.
                msg_embed_shape = (self.batch_size, self.max_n_days, self.max_n_msgs, self.msg_embed_size)
                # get message embeddings for one batch. reshape it.
    ...
    def _create_corpus_embed(self):
        """
            This function implements the attention mechanism that assigns weights to the messages
            to create one embedding per day, as a weighted average of the individual messages.
        ...
        with tf.name_scope('corpus_embed'):
            ...
            # u is a score for each day. set score if exists. else infinity
            masked_score = tf.where(mask_msgs, u, ninf)
            # batch norm, followed by softmax
            u = neural.softmax(masked_score)  # batch_size * max_n_days * max_n_msgs
            # replace nans by 0.0. weights for messages that will now sum to 1?.
    ...
    def _build_mie(self):
        """
            Create market information encoder. Multiplexing for fundamental, technical, etc.
    ...
    def _create_vmd_with_h_rec(self):
        with tf.name_scope('vmd'):
            with tf.variable_scope('vmd_h_rec'):
                # x is corpus embedding + price data. Note that they are directly 
                # working with x, and not looking at parts of it. So, we should be 
                # ok with just modifying the MIE portion of the mdoel.
    ...
    def _build_vmd(self):
        """
        High level function to build Variational Movement Decoder
        """
    ...
    def assemble_graph(self):
        """
        High level function to assemble all the components together, and create a consolidated model.
        """
    ...
    def _linear(self, args, output_size, activation=None, use_bias=True, use_bn=False):
        """
        Anish : This is a template for a linear layer
        Take args[0] or ags if not list/tuple. Then, get its shape. Puts -1 in shape dimensions where it is 0 (why tho?)
        x is args[0]. Some vector/matrix. acts as input to layer.
        A weight matrix W (parameter) is created with the above shape, and targeted output shape
        res = a tensor dot product of x, and W along axis 1.
        res is reshaped to desired size.
        batchnorm added if needed.
        activation applied if needed.
        res returned.
        """
