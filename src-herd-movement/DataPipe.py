...
from neighbor_dict import nbr_dict
...
class DataPipe:
    ...
    def _get_start_end_date(self, phase):
        """
        Each phase has a starting, and ending date. this just returns those dates according to phase.
    ...
    def sample_gen_from_one_stock(self, vocab_id_dict, stock_id_dict, s, phase):
        ...
        for main_target_date in main_target_dates:
            ...
            """
            We need price data for neighbor as well.
            """
            nbr_prices_and_ts = self._get_prices_and_ts(nbr_dict[s], main_target_date)
            # prices_and_ts = self._get_prices_and_ts(s_nbr, main_target_date)
            if not prices_and_ts :
                continue

            if not nbr_prices_and_ts:
                continue
            ...
            sample_dict = {
                ...
                # source
                # Edit here to concatenate price with neighbor's price.
                'prices': [prices_and_ts['prices'][i] + nbr_prices_and_ts['prices'][i] for i in range(len(prices_and_ts['prices']))],
    ...
    def batch_gen(self, phase):
        ...
        # we are creating a list of single stock generators.
        ...
        while True:
            ...
            price_batch = np.zeros([batch_size, self.max_n_days, 6], dtype=np.float32)
            ...
            while sample_id < batch_size:
                ...
                try:
                    ...
                    # T is the target day number. starting with 0? or 1?
                    T = sample_dict['T']
                    # meta
                    ...
                    # need to add the company_id variable here for tweet-scarcity case.
